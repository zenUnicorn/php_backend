...
if(isset($_POST['change-btn'])){
    ...
    while ($row_post=mysqli_fetch_array($run_select_query)){
        ...
        $user_id = $_SESSION['id'];
        $password = $row_post['user_password'];

            if($former_password == $password AND $id == $user_id){
                $updatepassword = "UPDATE users_table SET user_password = '$new_password' WHERE id= '$user_id'";
...
<head>
    ...
    <title>Environ Platform | User Profile</title>
    <!-- Favicon icon -->
